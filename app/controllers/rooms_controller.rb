...
class RoomsController < ApplicationController
  ...
  def check_user_can_start(subscribed, rooms)
    ...
    if rooms > 5 && subscribed == "Business"
      rooms_to_delete = rooms - 5
      $can_start = false
      return redirect_to root_path, alert: I18n.t("administrator.rooms.disallow_start", rooms: rooms_to_delete)
    end

    if rooms > 3 && subscribed == "Professional"
      # Number of rooms allowed is 3 in professional subscription
      # thus notify the user to delete certain number of existing rooms
      # Allow user to do so to avoid loss of data and improved UX
      rooms_to_delete = rooms - 3
      $can_start = false
      return redirect_to root_path, alert: I18n.t("administrator.rooms.disallow_start", rooms: rooms_to_delete)
    end
  end
